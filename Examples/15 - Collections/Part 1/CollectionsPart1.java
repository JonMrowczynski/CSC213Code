...
void ArrayListExamples() {
	IO.println("In ArrayListExamples:");
	var list = new ArrayList<Integer>(); // Don't need "Integer" twice, but kept here for demo.
	list.add(1);
	list.add(10);
	list.add(3);
	IO.println("\tArrayList after adding: " + list); // Can just directly print the data structure!
	IO.println("\tThe element at index 1 is: " + list.get(1));
	...
	IO.println("\tArrayList after removal: " + list);
	IO.println("\tThe element at index 1 is now: " + list.get(1));
	list.clear();
	IO.println("\tArrayList after clearing: " + list + "\n");
}
 ...
 * <p>
 * There are two main types of {@link LinkedList}s:
...
void LinkedListExamples() {
	IO.println("In LinkedListExamples:");
	...
	IO.println("\tLinkedList after adding: " + list); // Can just directly print the data structure!
	IO.println("\tThe element at index 1 is: " + list.get(1));
	...
	IO.println("\tLinkedList after removal: " + list);
	IO.println("\tThe element at index 1 is now: " + list.get(1));
	// Have special removeFirst and removeLast calls with a LinkedList
	list.removeFirst();
	list.removeLast();
	IO.println("\tLinkedList after clearing: " + list + "\n");
}
 ...
 * - Depending on the bucket implementation and the data types of the stored elements, this performance can be
 * either
 * O(k) or O(log(k)) where k is the number of elements in the bucket.
 */
void SetExamples() {
	IO.println("In SetExamples:");
	var set = new HashSet<Integer>();
	set.add(42);
	set.add(42); // Can't add multiple instances of the same thing!
	IO.println("\tHashSet after adding 42 twice: " + set);
	set.add(3);
	set.add(7);
	IO.println("\tHashSet doesn't guarantee insertion order: " + set + "\n");
}
 ...
 */
void MapExamples() {
	IO.println("In MapExamples:");
	var map = new HashMap<String, Double>();
	map.put("My First String", 42.0);
	map.put("PI", Math.PI);
	map.put("pi", Math.PI); // Remember case sensitivity!
	IO.println("HashMap after insertions: " + map);
	IO.println("Get a value with a key: " + map.get("PI"));
	IO.println("Get a value or a default if key not in Map: " + map.getOrDefault("E", Math.E));
	map.put("PI", 3.14);
	IO.println("HashMap after overwriting \"PI\" key: " + map + "\n");
}

/**
 * Similar to the utility class {@link Arrays} that contains a bunch of helper functions that helps us work with
 * arrays, we
 * have a utility class called {@link Collections} that helps us work with collections!
 */
void CollectionsExamples() {
	IO.println("In CollectionsExamples:");
	var list = new ArrayList<Integer>();
	Collections.addAll(list, 4, 8, 10, -1, -7, 5); // Can add a bunch of elements in one go.
	IO.println("\tList before sorting: " + list);
	Collections.sort(list); // Can efficiently sort the Collection!
	IO.println("\tList after sorting: " + list);
	IO.println("\tPrinting the elements out individually:");
	// Can use either an indexed for loop (if Collection is indexable) and/or a for each loop!
	for (var integer : list) { IO.println("\t\t" + integer); }
	IO.println();
}