/**
 * Contains a few examples of using enums and switches. 
 * 
 * Switches don't have to be used with enums, but it's rather common.
 * 
 * @author Jon Mrowczynski
 */
final String MONDAY_MSG = "Help me.";
final String MID_WEEK_MSG = "Just another day in the office.";
final String FRIDAY_MSG = "TGIF!";
final String WEEKEND_MSG = "Everybody's workin' for the weekend";
 ...
 */
final String DEFAULT_MSG = "Encountered unhandled Day! :O";

/**
 * - An enum can be thought of as a special kind of class where you know all possible states/instances ahead of time.
 ...
 * @author Jon Mrowczynski
 */
enum DaySimple { MONDAY, TUESDAY, WEDNESDAY, THURSDAY, FRIDAY, SATURDAY, SUNDAY; }

void main() {
    IO.println("Printing all possible values of " + DaySimple.class.getSimpleName() +":");
    for (var day : DaySimple.values()) { 
        IO.println(day + "'s ordinal value is: " + day.ordinal()); 
    }
    IO.println();
    ...
    IO.println(switchExpressionWithReturnedValue(day));
}
 ...
 * @param day the {@link DaySimple} used for demo purposes.
 */
void switchStatement(DaySimple day) {
    switch(day) {
        case MONDAY:
            IO.println(MONDAY_MSG);
            break;
 ...
 * @param day the {@link DaySimple} used for demo purposes.
 */
void switchExpressionWithVoidReturn(DaySimple day) {
    switch(day) {
        case MONDAY -> IO.println(MONDAY_MSG);
        case TUESDAY, WEDNESDAY, THURSDAY -> IO.println(MID_WEEK_MSG);
        case FRIDAY -> IO.println(FRIDAY_MSG);
        case SATURDAY, SUNDAY -> IO.println(WEEKEND_MSG);
        // Not needed if there is a case for all possible values! 
        // Although good to have in case the enum is expanded later in development.
        default -> IO.println(DEFAULT_MSG);
    }
}

/**
 * Functionally similar to other functions, but entirely removes the repetitive print statements!. 
 * 
 * Also, increases flexibility in terms of what is done with the message since we aren't printing it by default.
 * 
 * @param day the {@link DaySimple} used for demo purposes.
 * @return a {@link String} whose message is based on the value of {@code day}.
 */
String switchExpressionWithReturnedValue(final DaySimple day) {
    return switch(day) {
